...
class _ModuleBank(object):
    ...
    def _get_type_hints(self, classname, instance, method_name, cardinality):
        hints = [iris._Constant.MetaType_VARIANT]*cardinality
        try:
            if instance != None:
    ...
    def _convert_type_to_meta(self, typeobject):
        try:
            if typeobject == bool:
                return iris._Constant.MetaType_BOOL
            if typeobject == int:
                return iris._Constant.MetaType_INTEGER
            if typeobject == float:
                return iris._Constant.MetaType_DOUBLE
            if typeobject == decimal.Decimal:
                return iris._Constant.MetaType_DECIMAL
            if typeobject == bytes:
                return iris._Constant.MetaType_BYTES
            if typeobject == str:
                return iris._Constant.MetaType_STRING
            if typeobject == datetime.date:
                return iris._Constant.MetaType_DATE
            if typeobject == datetime.time:
                return iris._Constant.MetaType_TIME
            if typeobject == datetime.datetime:
                return iris._Constant.MetaType_DATETIME
            if typeobject == iris.IRISList:
                return iris._Constant.MetaType_IRISLIST
            else:
                return iris._Constant.MetaType_VARIANT
        except Exception as ex:
            pass
        return iris._Constant.MetaType_VARIANT
